...
Find covid death weighted centroids at the state level based on weekly case counts at the municaplity level. 
...
for (i in start_week_id:end_week_id){
      ...
      centroids_temp <- state_weighted_centroid(spat_temp, "state_code", "sum_deaths", i, s)
      ...
      ###for previous week###
              ...
              centroids_temp_prev <- state_weighted_centroid(spat_temp_prev, "state_code", "sum_deaths",a, s)
        ...
        weekly_cent_temp  <- st_as_sf(data.frame(period = paste0(a," - ",i),
    ...
    weekly_line_merge <- rbind(weekly_line_merge,weekly_cent_temp) %>% filter(!is.na(period)) %>% filter(period!="9_9") %>% st_set_crs(.,29101) %>% st_transform(.,29101)
      ...
      w27_dots <- rbind(w27_dots,centroids) %>% filter(!is.na(state_code))
        ...
        maxval_temp <- max(spat_temp$sum_deaths)
        
      if (maxval_temp < 101){
        maxval_temp2.nm <- 102
        maxval_temp2.chr <- "> 101"

        
        } else{
        maxval_temp2.nm <- maxval_temp
        maxval_temp2.chr <- maxval_temp %>% formatC(., format="f", big.mark=",", digits=0) 
        maxval_temp2.chr <- paste0("101 - ", maxval_temp2.chr)
      ...
      nbin <- tm_shape(state_temp) + tm_borders(col="gray60") + tm_shape(spat_temp) + tm_borders(col="gray60", lwd=0.1) + tm_fill(col="sum_deaths", palette = "OrRd", breaks=c(0,0.1,10.1,100.1,maxval_temp2.nm), title = "Total Deaths", alpha=0.5, labels = c ("0","1 - 10", "11 - 100",maxval_temp2.chr)) + tm_layout(main.title = paste0("COVID-19 Death-Weighted Centroid for ",state_name_temp$NM_ESTADO, ", Brazil"), main.title.size = 1.2, main.title.position = "center", frame=FALSE, legend.outside = TRUE) + tm_shape(dist_temp2) + tm_lines(col="Period", lwd=3, palette = linecols4) + tm_shape(w27_dots_temp) + tm_bubbles(size=0.5, border.col="red", border.lwd=1.5, border.alpha=1, col="white", alpha=0.1) + tm_shape(capital_plot) + tm_dots(col="black", size = 0.3)
          + tm_scale_bar(breaks = c(0,50,100), color.dark = "gray60", position = c("right", "bottom"),just="right", text.color = "gray60")
            
         print(nbin)
          
         #tmap_save(nbin, paste0("/Users/erin/Desktop/Harvard_NSF_REU/Brazil_COVID/deaths/state_files/state_",s,"week",i,".png"))
...
dir5 <- dir3 %>% mutate(length_cents = ifelse(geo_st_x == x_pi, 0, length_cents)) %>% mutate(azimuth = ifelse(geo_st_x == x_pi, 0, azimuth)) %>% mutate(bearing = ifelse(geo_st_x == x_pi, 0, bearing)) 
      ...
      write.csv(state_export,paste0("/Users/erin/Desktop/Harvard_NSF_REU/Brazil_COVID/deaths/state_files/data/States_lines_data_extended.csv") , row.names = FALSE)
      

      state_export_shp <- dir5 %>% st_transform(29101) %>% select(!bearing) %>% select(!geo_st_x) %>% select(!geo_st_y) %>% select(!cap_line)
        
      st_write(state_export_shp, paste0("/Users/erin/Desktop/Harvard_NSF_REU/Brazil_COVID/newdata/state_deaths.shp"), drive="ESRI Shapefile")
...
(Also created in ArcGIS using data from above code.)
```{r}
linecols4 <- c("#73B2FF", #9
              "#6FA4FE", #10
              "#6B96FD", #11
